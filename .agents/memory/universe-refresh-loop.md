...
**Ek (2026-07-31):** `save_smart_config` seed dosyasına ASLA yazmaz — kanonik durum `alpha20_v1/universe_runtime.json` içindeki `smart` overlay'idir. `scheduler_refresh`'i seed `smart_config.json`'dan okumak yanıltır (hep null görünür); doğrulama overlay'den yapılmalı. Ayrıca controller loop'u post_fork yalnız `adaptive_system.enabled=true` iken başlatır; Operation START tek başına scheduler'ı canlandırmaz — zincir: operation START + adaptive enable (/adaptive/enable kanonik yolu).
