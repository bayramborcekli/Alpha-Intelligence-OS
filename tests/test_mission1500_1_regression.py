...
class TestRouteSurface:
    ...
    ALLOWED_WRITE_ROUTES = {
        ...
        "/api/lab/control",
        # Task 145/155-157: ORPHAN + kapatılamaz eksik legacy kaydın
        # state.json'dan güvenli temizliği (CSRF+auth+rate-limit;
        # bot çalışırken 409 CLEANUP_REQUIRES_CONTROLLER_PAUSE; flock
        # altında taze yeniden sınıflandırma; yalnız temizlenebilir
        # kanıtlı kayıt silinir — bilinçli genişletme)
        "/api/state/orphan-position/clean",
        # Task 149/152: INCOMPLETE kaydın operatör onayı (audit'e
        # operator_ack; dual kaynağı runtime'dan çıkarır — bilinçli
        # genişletme)
        "/api/positions/integrity/ack",
    }
